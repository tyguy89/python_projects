...
class RGBShapeDetection:
    def __init__(self):
        ...
        self.reversed_index = {}
        for k in self.index:
            self.reversed_index[str(self.index[k])] = k
    ...
    def find_colour_in_image(self, img):
    ...
    def is_edge_of_shape(self, array):
        id = array[1][1]
        #print(id, "id")
        counter = 0
        for a in array:
        ...
        for i in range(len(filtered_img)):
            for j in range(len(filtered_img[i])):
                ...
                if self.is_edge_of_shape(ninebynine):
                    ...
                    edge_colours[self.reversed_index[str(filtered_img[i][j])]].append((i, j))
